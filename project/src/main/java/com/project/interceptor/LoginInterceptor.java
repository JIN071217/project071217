...
public class LoginInterceptor extends HandlerInterceptorAdapter {@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)throws Exception{
		...
		//로그인 세션이없는 경우, 로그인 화면 출력
		if(loginAuth == null) {
			
			if(isAjaxRequest(request)) {
				
				//AJAX요청은 로그인 JSP를 사용불가능. 이유는 리턴ㄱ밧만 부분 업데이트 성격으로 받는 특징때문에,
				//response.sendRedirect("member/login");
				
				response.sendError(400);
			}else {
				
				//사용자가 로그인으로 강제진행되기전 요청했던 주소를 저장하는 작업. (원래 요청된 주소)
		...
		return true; // 인증된 사용자 일 경우, 요청한 컨트롤러의 매핑주소로 제어가넘어간다.
	}
	...
	private boolean isAjaxRequest(HttpServletRequest request) {
		...
		String header = request.getHeader("AJAX"); //클라이언트 AJAX호출하는 부분에서 작업이 선행되어야한다.
